...
class DeepSAD(object):
    ...
    def __init__(self, eta: float = 1.0):
        ...
        self.results = {
            'train_time': None,
            'test_auc': None,
            'test_aupr': None,  # Added AUPR metric
            'test_time': None,
            'test_scores': None,
        }
        self.ae_results = {
    ...
    def test(self, dataset: BaseADDataset, device: str = 'cuda', n_jobs_dataloader: int = 0):
        ...
        # Calculate AUPR
        if self.results['test_scores'] is not None:
            _, labels, scores = zip(*self.results['test_scores'])
            self.results['test_aupr'] = average_precision_score(labels, scores)
    ...
    def save_epoch_results(self, export_json):
        epoch_results = {
            'train_losses': self.trainer.train_losses,
            'test_aucs': self.trainer.test_aucs
        }
        with open(export_json, 'w') as fp:
            json.dump(epoch_results, fp)

    def save_dsamples(self,dataset: BaseADDataset, export_json, drange=0.5):
        if self.results['test_scores'] is not None:
            image_name, labels, scores = zip(*self.results['test_scores'])
            Normal_max_score = max([scores[i] for i in range(len(labels)) if labels[i] == 0 ],default=None)
            d_idx = [i for i in range(len(scores)) if labels[i]==1 and scores[i]<=Normal_max_score+drange]
            captions = dataset.getdcaptions(d_idx)
            d_results = [self.results['test_scores'][i] for i in d_idx]
            with open(export_json, 'w') as fp:
                json.dump(list(zip(d_results,captions)), fp)

    def save_esamples(self,dataset: BaseADDataset, export_json, num=59):
        if self.results['test_scores'] is not None:
            image_name, labels, scores = zip(*self.results['test_scores'])
            labels_1_idx = [i for i,lb in enumerate(labels) if lb ==1]
            scores_of_lb1 = [(i,scores[i]) for i in labels_1_idx]
            sorted_scores_of_lb1 = sorted(scores_of_lb1,key=lambda x: x[1],reverse=True)
            d_idx = [i for i,score in sorted_scores_of_lb1[:num]]
            # Normal_max_score = max([scores[i] for i in range(len(labels)) if labels[i] == 0 ],default=None)
            # d_idx = [i for i in range(len(scores)) if labels[i]==1 and scores[i]<=Normal_max_score+drange]
            captions = dataset.getdcaptions(d_idx)
            d_results = [self.results['test_scores'][i] for i in d_idx]
