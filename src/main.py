...
################################################################################
@click.command()
@click.option('--dataset_name', type=click.Choice(['flower','ucm_caption','mycifar']), default="flower", help='Dataset name to process.')
@click.option('--net_name', type=click.Choice(['mnist_LeNet', 'fmnist_LeNet', 'cifar10_LeNet', 'arrhythmia_mlp',
                                               ...
                                               'thyroid_mlp']),default="cifar10_LeNet",help='Network name to use.')
@click.option('--xp_path', type=click.Path(exists=True),default='/home/zhengqing/KS/flower_models', help='Path to save experiment results.')
@click.option('--data_path', type=click.Path(exists=True),default='/home/zhengqing/KS/Deep-SAD-PyTorch/data/flowers/flower102_caption', help='Path to the dataset.')
...
@click.option('--ratio_known_normal', type=float, default=0, help='Ratio of known (labeled) normal training examples.')
@click.option('--ratio_known_outlier', type=float, default=0.01, help='Ratio of known (labeled) anomalous training examples.')         
@click.option('--ratio_pollution', type=float, default=0.1, help='Pollution ratio of unlabeled training data with unknown (unlabeled) anomalies.')
...
@click.option('--n_epochs', type=int, default=200, help='Number of epochs to train.')
...
@click.option('--ae_n_epochs', type=int, default=200, help='Number of epochs to train autoencoder.')
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, eta,
    ...
    # Need to pre-train the ae (self-coding) model first
    if pretrain:
    ...
    deepSAD.save_epoch_results(export_json=xp_path + '/epoch_results.json')
    deepSAD.save_dsamples(dataset,export_json=xp_path + '/d_samples.json', drange=0)
    deepSAD.save_esamples(dataset, export_json=xp_path + '/e_samples.json', num=100)
    # cfg cannot be saved directly
